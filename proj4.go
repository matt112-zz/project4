...
func writeBytecode(mem []byte, bytecode string) {
	log.Println("writeBytecode() with ", bytecode)
...
func fetch(mem []byte) Instruction {
	...
	// log.Println("With ins(", ins, ") op1(", op1, ") op2(", op2, ")")
	return IR
}

func fetchInt(mem []byte) int32 {
	// log.Println("fetchInt()")
	var v int32
...
func firstpass(filename string) {
	log.Println("~~~~~~~~~~~~~Start of first pass~~~~~~~~~~~~~~~~")
	...
	for scanner.Scan() {
		...
		log.Println("First pass with line: ", line, " at memory location : ", count)

		// traps and such
		if len(tokens) == 2 {
			log.Println("There are two tokens")
			if _, ok := instruction_table[tokens[0]]; ok {
				// log.Println("found ", tokens[0], " in instruction table!")
			...
			log.Println("There are 3 tokens")
			if _, ok := instruction_table[tokens[0]]; ok {
				count += INSTRUCTION_SIZE
			} else if _, ok := instruction_table[tokens[1]]; ok {
				// log.Println("found ", tokens[0], " in instruction table!")
				if _, ok := symbol_table[tokens[0]]; !ok {
					// log.Println("Label not in symbol table. Label for an instruction.")
				...
				if _, ok := symbol_table[label]; !ok {
					// log.Println("Label not in symbol table")
			...
			log.Println("There are 4 tokens")
			label = tokens[0]
			if _, ok := symbol_table[label]; !ok {
				symbol_table[label] = count
				count += INSTRUCTION_SIZE
	...
	log.Println("~~~~~~~~~~~~~~~~~End of first pass~~~~~~~~~~~~~~")

}

func secondpass(filename string) {
	log.Println("~~~~~~~~~~~~Start of second pass~~~~~~~~~~~~~~~")
	...
	for scanner.Scan() {
		...
		log.Println("Second pass with line: ", line, " at memory location : ", count)

		// traps and such
		if len(tokens) == 2 {
			log.Println("There are 2 tokens")
	...
	log.Println("~~~~~~~~~~~~~End of second pass~~~~~~~~~~~~~~~~~~")
}

func instruction_to_bytecode(instruction string, operands []string) string {
	log.Println("instruction_to_bytecode() with instruction(", instruction, ") and operands(", operands, ")")
	var bytecode string
	...
	case "DIV":
		return fmt.Sprintf("%d %d %d", instruction_table[instruction], register_table[operands[0]], register_table[operands[1]])
	case "AND":
		log.Println("AND not implemented")
	case "OR":
		log.Println("OR not implemented")
	case "CMP":
		return fmt.Sprintf("%d %d %d", instruction_table[instruction], register_table[operands[0]], register_table[operands[1]])
	case "TRP":
...
func virtualmachine() {
	log.Println("~~~~~~~~~~~~~~~~~Running Virtual Machine~~~~~~~~~~~~~~")
	...
	log.Println("Stack at location(", stack, ") in memory")
	...
	for !done {
		...
		case instruction_table["TRP"]:
			...
			if mode == 0 {
				...
				log.Println("~~~~~~~~~~~~~~~Register 3: ", string(registers[3]))
				kb_buff = kb_buff[1:]
			} else if mode == 99 {
				// TRP 99 used for debuggging
				log.Println("****************DEBUGGING LINE ", PC_to_ASM_Line[PC+12], " OF ASSEMBLY CODE****************")
			} else if mode == 98 {
				log.Println(fetchInt(memory[MEMORY_SIZE-8:]))
			}
		// JUMP INSTRUCTIONS
		case instruction_table["JMP"]:
			...
			log.Printf("JMP instruction: Branching to label at location(%d)\n", l_addr)
			// printMemory()
		case instruction_table["JMR"]:
			branch = true
			l_addr = registers[IR.opd1]
			registers[register_table["PC"]] = l_addr
			log.Printf("JMR instruction: Branching to address in s_reg(%d)\n", l_addr)
		case instruction_table["BNZ"]:
			...
			log.Printf("BNZ instruction: Branching to label at location(%d) if s_reg(%d) != 0\n", l_addr, s_reg_name)
		case instruction_table["BGT"]:
			...
			log.Printf("BGT instruction: Branching to label at location(%d) if s_reg(%d) > 0\n", l_addr, s_reg_name)
		case instruction_table["BLT"]:
			...
			log.Printf("BLT instruction: Branching to label at location(%d) if s_reg(%d) < 0\n", l_addr, s_reg_name)
		case instruction_table["BRZ"]:
			...
			log.Printf("BRZ instruction: Branching to label at location(%d) if s_reg(%d) = 0\n", l_addr, s_reg_name)
		...
		case instruction_table["MOV"]:
			...
			log.Printf("MOV instruction: Moving s_reg(%d) into d_reg(%d)\n", s_reg_name, d_reg_name)
		case instruction_table["LDA"]:
			d_reg = IR.opd1
			l_addr = IR.opd2
			registers[d_reg] = l_addr
			log.Printf("LDA instruction: Loading address of label at location(%d) in d_reg(%d)\n", l_addr, d_reg)
		case instruction_table["STR"]:
			s_reg_name = IR.opd1
			l_addr = IR.opd2
			writeInt(memory[l_addr:], strconv.Itoa(int(registers[s_reg_name])))
			log.Printf("STR instruction: Storing data into Mem/Label at location(%d) in s_reg(%d)\n", l_addr, s_reg_name)
			// printMemory()
		case instruction_table["LDR"]:
			d_reg = IR.opd1
			s_addr = IR.opd2
			registers[d_reg] = fetchInt(memory[s_addr:])
			log.Printf("LDR instruction: Loading d_reg(%d) with data from Mem/Label at location(%d)\n", d_reg, s_addr)
		case instruction_table["STB"]:
			s_reg = IR.opd1
			s_addr = IR.opd2
			memory[s_addr] = byte(registers[s_reg])
			log.Printf("STB instruction: Storing byte into Mem/Label at location(%d) from s_reg(%d)\n", s_addr, s_reg)
		case instruction_table["LDB"]:
			d_reg = IR.opd1
			s_addr = IR.opd2
			registers[d_reg] = int32(memory[s_addr])
			log.Printf("LDB instruction: Loading byte into d_reg(%d) from Mem/Label at location (%d)\n", d_reg, s_addr)
		...
		case instruction_table["ADD"]:
			...
			log.Println("ADD instructions")
		case instruction_table["ADI"]:
			d_reg_name = IR.opd1
			registers[d_reg_name] += IR.opd2
			log.Printf("ADI instruction: Adding immediatte(%d) to d_reg(%d)\n", IR.opd2, d_reg_name)
		...
		case instruction_table["DIV"]:
			...
			s_reg = registers[s_reg_name]
			registers[d_reg_name] = d_reg / s_reg
			log.Println("DIV instructions")

		// LOGICAL INSTRUCTIONS
		case instruction_table["AND"]:
			log.Println("AND not implemented in VM")
		case instruction_table["OR"]:
			log.Println("OR not implemented in VM")
		...
		case instruction_table["STRI"]:
			...
			log.Printf("STRI instruction: Storing data from s_reg(%d) to memory at RG(%d)\n", s_reg_name, g_reg_name)
		case instruction_table["LDRI"]:
			d_reg_name = IR.opd1
			g_reg_name = IR.opd2
			m_addr = registers[g_reg_name]
			registers[d_reg_name] = fetchInt(memory[m_addr:])
			log.Printf("LDRI instruction: Load d_reg(%d) with data from memory at d_reg(%d)\n", d_reg_name, g_reg_name)
	...
	log.Println("~~~~~~~~~~~~~~End of virtualmachine()~~~~~~~~~~~~~~~~~~")
}
...
func main() {
	...
	// log.Println("Start of main program")
	filename := "proj4.asm"
	firstpass(filename)
	secondpass(filename)
	// printMemory()
